<template>
  <div class="av__visualizer">
    ...
    @prevAudio="$emit('prevAudio')"
    @nextAudio="$emit('nextAudio')"
...
export default {
  ...
  methods: {
    ...
    mainLoop () {
      ...
      const numBars = 64
      const gapSize = 4
      const step = Math.round(frqBits * 0.85 / (numBars)) // Audios do not contain the higher frequencies so cut them out
      var xPos = 0
